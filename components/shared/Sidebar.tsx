...
export const Sidebar = () => {
  ...
  const { data: profile, status, error } = useGetProfile(session);
    ...
    <div className="h-screen w-20 md:w-60 px-2 md:px-6 flex flex-col overflow-y-scroll scrollbar-hidden">
      ...
      <div className="mt-4 mb-14 h-14 flex items-center md:space-x-2 max-md:justify-center md:h-12 relative">
        ...
        <div className="relative group max-md:hidden flex-1">
          {/* Tooltip */}
          <div className="absolute pointer-events-none opacity-0 bottom-full w-[125%] left-1/2 -translate-x-1/2 p-2 bg-white rounded-md group-hover:opacity-100 group-hover:pointer-events-auto group-hover:bottom-[175%] transition-[opacity,bottom] ring-1 ring-secondary drop-shadow-sm duration-300">
            {/* Bottom Arrow */}
          ...
          <p className="text-sm line-clamp-1 break-all text-center">
            {profile?.name}
          </p>
        </div>
      </div>
